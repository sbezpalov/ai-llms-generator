...
def dump_signals(text: str) -> list[str]:
    ...
    link_count = len(LLMS_LINK_RE.findall(text))
    if link_count > 20:
        signals.append(f"too-many-links:{link_count}")
    # Any single H2 with >12 links already fails validate_llms_shape; dump
    # fixtures may violate that on purpose — count sections over 12 here too.
    section_links = 0
